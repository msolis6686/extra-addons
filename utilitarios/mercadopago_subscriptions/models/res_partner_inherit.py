...
class mercadopago_subscriptions(models.Model):
    ...
    subscription_type = fields.Many2one('mercadopago.subscriptions', string='Tipo de Subscripcion:')
    ...
    def compose_subscription_mercadopago(self):
        ...
        template = self.env.ref(
            'mercadopago_subscriptions.email_template_mercadopago_subscription', False)
        if not template:
            template_id = False
        else:
    ...
    def prepare_mass_subscription(self):
        ...
        view_id = self.env.ref('mercadopago_subscriptions.bf_mp_mass_subscription').id#Este dato lo sacamos de Ajustes/Tecnico/Vistas. Es el ID externo
